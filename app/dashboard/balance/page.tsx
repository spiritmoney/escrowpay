    ...
    <Dialog>
      ...
      <DialogContent className="sm:max-w-[425px] p-4 md:p-6 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="grid gap-4 py-2 md:py-4">
          ...
          <Button
            type="submit"
            className="w-full h-10 md:h-11 mt-2 bg-blue-600 hover:bg-blue-700 text-white"
          >
        ...
        <form onSubmit={handleSubmit} className="grid gap-4 py-2 md:py-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, payerEmail: e.target.value }))
              }
            />
          </div>
          <div className="space-y-2">
...
const ConvertCurrencyModal = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage =
        error.message ||
        "Failed to convert currency or Insufficient balance for conversion";
      toast.error(errorMessage);
    }
  };
          ...
          <div className="pt-2">
            {conversionRate && (
              <p className="text-sm md:text-base text-gray-500">
                Estimated rate: 1 {formData.from} ≈ {conversionRate}{" "}
                {formData.to}
              </p>
            )}
          </div>
...
const BalancePage: React.FC = () => {
  try {
    ...
    if (isBalancesLoading || isActivityLoading)
      return (
        <DashboardLayout>
    ...
    // Handle empty state - show UI with zero balances instead of error
    if (!balances) {
      return (
        <DashboardLayout>
          <div className="p-4 md:p-6">
            <Alert>
              ...
              <AlertDescription>
                No balance information is currently available. Please try again
                later.
              </AlertDescription>
            </Alert>
          </div>
    ...
    const fiatBalances = [
      {
        name: "NGN",
        amount: `₦${(balances.fiat?.NGN || 0).toLocaleString()}`,
        change: `${balances.fiat?.NGN_change || 0}%`,
        icon: <Wallet className="h-4 w-4 text-blue-600" />,
      },
      {
        name: "USD",
        amount: `$${(balances.fiat?.USD || 0).toLocaleString()}`,
        change: `${balances.fiat?.USD_change || 0}%`,
        icon: <Wallet className="h-4 w-4 text-blue-600" />,
      },
      {
        name: "EUR",
        amount: `€${(balances.fiat?.EUR || 0).toLocaleString()}`,
        change: `${balances.fiat?.EUR_change || 0}%`,
        icon: <Wallet className="h-4 w-4 text-blue-600" />,
      },
      {
        name: "GBP",
        amount: `£${(balances.fiat?.GBP || 0).toLocaleString()}`,
        change: `${balances.fiat?.GBP_change || 0}%`,
        icon: <Wallet className="h-4 w-4 text-blue-600" />,
      },
    ];
    ...
    const cryptoBalances = [
      {
        name: "Espees (ESP)",
        amount: `${balances.crypto?.ESP?.amount || 0} ESP`,
        value: `$${(balances.crypto?.ESP?.usdValue || 0).toLocaleString()}`,
        change: `${balances.crypto?.ESP_change || 0}%`,
        icon: <Coins className="h-4 w-4 text-blue-600" />,
      },
      {
        name: "USDC",
        amount: `${balances.crypto?.USDC?.amount || 0} USDC`,
        value: `$${(balances.crypto?.USDC?.usdValue || 0).toLocaleString()}`,
        change: `${balances.crypto?.USDC_change || 0}%`,
        icon: <Coins className="h-4 w-4 text-green-600" />,
      },
      {
        name: "USDT",
        amount: `${balances.crypto?.USDT?.amount || 0} USDT`,
        value: `$${(balances.crypto?.USDT?.usdValue || 0).toLocaleString()}`,
        change: `${balances.crypto?.USDT_change || 0}%`,
        icon: <Coins className="h-4 w-4 text-green-600" />,
      },
    ];
      ...
      <DashboardLayout>
        <div className="p-4 md:p-6">
          <Alert variant="destructive">
            ...
            <AlertTitle>Error</AlertTitle>
            <AlertDescription>
              An unexpected error occurred while loading the balance page.
